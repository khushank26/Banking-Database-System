...
long long LinearProbing::pow(int b, int p, int s)
{
...
void LinearProbing::merge(vector<int> &arr, int left, int mid, int right)
{
    int n1 = mid - left + 1;
    int n2 = right - mid;
...
void LinearProbing::mergeSort(vector<int> &arr, int left, int right)
{
    if (left < right)
    {
...
bool LinearProbing::deleteAccount(std::string id)
{
    ...
    for (int i = 0; i < v1.size(); i++)
    {
        if (v1[i] == bankStorage1d[k].balance)
        {
            v1.erase(v1.begin() + i);
            break;
        }
